        ...
        <div class="mx-auto max-w-7xl px-6 pb-8 pt-16 sm:pt-24 lg:px-8 lg:pt-12">
                    ...
                    <div class="md:grid md:grid-cols-4 md:gap-8">
                        <div v-for="linkGroup in linkGroups">
                            ...
                            <ul role="list" class="mt-6 space-y-4">
                                <li v-for="item in linkGroup.links" :key="item.name">
            ...
            <div class="mt-8 border-t border-white/10 pt-8 md:flex md:items-center md:justify-between">
                <div class="flex space-x-6 md:order-2" v-if="false">
                    <a v-for="item in navigation.social" :key="item.name" :href="item.href" class="text-primary-foreground">
                        <span class="sr-only">{{ item.name }}</span>
                        <component :is="item.icon" class="h-6 w-6" aria-hidden="true" />
                    </a>
                </div>
                <p class="mt-8 text-xs leading-5 text-primary-foreground md:order-1 md:mt-0">&copy; {{ $page.props.globals.general.copyright }}</p>
            </div>
        </div>
    </footer>
</template>
...
const linkGroups = usePage().props.globals.footer.linkgroups;
